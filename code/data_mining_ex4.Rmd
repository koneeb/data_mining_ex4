...
I did not explore this technique any further since the average Silhoutte score is so much lower than that of the kmeans++. However, I have displayed the average values for the different clusters below. 
...
# summarise results of k=6 agglomerative clustering
hclust_groups <- cutree(hier_mkt_ward, k=6)

mkt %>%
  mutate(hclust_groups) %>%
  group_by(hclust_groups) %>% 
  summarise_all('mean')

# # plot the dendrogram
# plot(hier_mkt_ward, cex=0.8, hang=-1)
```
...
knitr::include_graphics(here('graphs/gephi_NA.png'))
```